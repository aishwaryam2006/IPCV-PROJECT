...
FONT_SCALE  = 0.55
THICKNESS   = 2
WINDOW_NAME = "Before | After  –  press Q to quit"
...
def preprocess_frame(frame):
    ...
    # 1. Resize to 320x240 then scale back (shows pixelation effect)
    ...
    hist_eq = cv2.merge([b_eq, g_eq, r_eq])

    # 3. Gaussian Blur
    blurred = cv2.GaussianBlur(hist_eq, (5, 5), 1)
...
def run(source=0, use_ssd=True, use_hog=True):
    ...
    while True:
        ...
        frame_count += 1
        raw_frame = frame.copy()  # clean copy BEFORE drawing
        ...
        # ─ Preprocess the clean frame ─
        processed = preprocess_frame(raw_frame)

        # ─ Add labels ─
        pad = 30
        before = cv2.copyMakeBorder(frame,     pad, 0, 0, 0, cv2.BORDER_CONSTANT, value=(30, 30, 30))
        after  = cv2.copyMakeBorder(processed, pad, 0, 0, 0, cv2.BORDER_CONSTANT, value=(30, 30, 30))
        cv2.putText(before, "BEFORE (Raw + Detections)", (10, 22), FONT, 0.65, (255, 255, 255), 2)
        cv2.putText(after,  "AFTER  (Blur + CLAHE)",     (10, 22), FONT, 0.65, (0, 255, 150),   2)
